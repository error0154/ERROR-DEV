...
TOKEN = '7396505612:AAHEWA4VWGGcvJjl9ik17jglWz5p9bvyRuc'
...
def handle_attack_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "*💣JALDI ATTACK MAAR BAHI KISI NE BHI NI LAGA RAKHA *\n"  # Ready to launch message
                                   ...
                                   "*OWNER @error_userrr🧛*", parse_mode='Markdown')  # Start chaos message
...
def myinfo_command(message):
    user_id = message.from_user.id
    user_data = users_collection.find_one({"7871547719": user_id})

    if not user_data:
        # User not found in the database
        response = "*❌ Oops! No account information found!* \n"  # Account not found message
        response += "*For assistance, please contact the owner: @error_userrr* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @error_userrr 🙏"  # Contact owner message
    else:
...
def start_message(message):
    try:
        bot.send_message(message.chat.id, "*🌍 WELCOME TO ERROR FREEZE DDOS!* 🎉\n\n"
...
if __name__ == "__main__":
    ...
    logging.info("Bot started join @error_ddos1")
    while True:
        try:
